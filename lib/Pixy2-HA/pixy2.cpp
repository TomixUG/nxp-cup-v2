...
/* Le programme utilise l'interruption de la liaison série pour recevoir le message et avancer la machine d'état pour permettre au programme de ne pas être bloquant en réception.
   ...
   **** COMMENT CA MARCHE ****
   ...
   A partir de ça, on attend de recevoir les 4 premiers octets de l'entête :
   ...
   * les données (hPointer + 6 et +)
   Et si on a une trame sans checksum (pas normal) :
   * les données (hPointer + 4 et +)
...
void PIXY2::pixy2_getByte() // Interruption de la pixy2
{
    ...
    case messageSent: // Si on a envoyé une requete => on attend un entête
        if (wPointer > 0)
        {                                                   // On attend d'avoir reçu 2 octets
...
T_pixy2ErrorCode PIXY2::pixy2_getVersion(T_pixy2Version *version)
{
    ...
    case idle:                             // Si la caméra est inactive
        ...
        if (cr != PIXY2_OK)
            return cr;      // S'il y a une erreur lors de l'envoi on ejecte !
    ...
    case dataReceived: // Quand on a reçu l'intégralité du message
        ...
        { // Si la trame contient un checksum
            if (!pixy2_validateChecksum(&Pixy2_buffer[hPointer]))
            {                              // On lance la validation du checksum
                return PIXY2_BAD_CHECKSUM; // Si le checksum est faux on retourne une erreur
            }
        }
        if (msg->pixType == PIXY2_REP_VERS)
        {                                                        // On vérifie que la trame est du type convenable (REPONSE VERSION)
        ...
        { // Si ce n'est pas le bon type
            if (msg->pixType == PIXY2_REP_ERROR)
            {                                                      // Cela pourrait être une trame d'erreur
                cr = *(T_pixy2ErrorCode *)&Pixy2_buffer[dPointer]; // Si c'est le cas, on copie le code d'erreur reçu dans la variable de retour
            }
            else
                cr = PIXY2_TYPE_ERROR; // Si le type ne correspond à rien de normal on signale une erreur de type.
        }
...
T_pixy2ErrorCode PIXY2::pixy2_getResolution(T_pixy2Resolution *resolution)
{
    ...
    case dataReceived: // Quand on a reçu l'intégralité du message
        ...
        {                                                              // On vérifie que la trame est du type convenable (REPONSE RESOLUTION)
...
T_pixy2ErrorCode PIXY2::pixy2_getFPS(T_pixy2ReturnCode *framerate)
{
    ...
    case dataReceived: // Quand on a reçu l'intégralité du message
        ...
        {                                                             // On vérifie que la trame est du type convenable (REPONSE FPS)
...
T_pixy2ErrorCode PIXY2::pixy2_getBlocks(Byte sigmap, Byte maxBloc)
{
    ...
    case dataReceived: // Quand on a reçu l'intégralité du message
        ...
        if (msg->pixType == PIXY2_REP_BLOC)
        {                                                          // On vérifie que la trame est du type convenable (REPONSE BLOCS)
...
T_pixy2ErrorCode PIXY2::pixy2_getFeatures()
{
    ...
    if (msg->pixType == PIXY2_REP_LINE)
    {                        // On vérifie que la trame est du type convenable (REPONSE LIGNE)
        ...
        do
        {
            ...
            if (lineFeature->fType == PIXY2_VECTOR)
            { // On regarde si le type est vecteur
                Pixy2_numVectors = lineFeature->fLength / sizeof(T_pixy2Vector);
                // Si oui, on compte combien il y a de vecteurs
                Pixy2_vectors = (T_pixy2Vector *)&Pixy2_buffer[fPointer]; // On mappe le résultat
                fPointer += lineFeature->fLength;                         // On déplace le pointeur de données et on recommence
                cr |= PIXY2_VECTOR;
            }

            if (lineFeature->fType == PIXY2_BARCODE)
            { // On regarde si le type est codebarre
            ...
            if (lineFeature->fType == PIXY2_INTERSECTION)
            { // On regarde si le type est intersection
...
T_pixy2ErrorCode PIXY2::pixy2_getRGB(Word x, Word y, Byte saturate, T_pixy2Pixel *pixel)
{
    ...
    case dataReceived: // Quand on a reçu l'intégralité du message
        ...
        if (msg->pixType == PIXY2_REP_ACK)
        {                                                    // On vérifie que la trame est du type convenable (REPONSE ACK)
...
T_pixy2ErrorCode PIXY2::pixy2_validateChecksum(Byte *tab)
{
    ...
    for (i = 0; i < *(tab + 3); i++)
        sum = sum + *(tab + PIXY2_CSHEADERSIZE + i);
    tmp = (T_Word *)(tab + 4);
    if (tmp->mot == sum)
        return PIXY2_OK;
    else
